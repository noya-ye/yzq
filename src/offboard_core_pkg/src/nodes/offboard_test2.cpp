...
//   {
//     // 只有 YawSpinTask 开启 D435 门控时，才接受深度相机目标点
...
//         // ========================================================
//         // 关键：
//         // 通知 YawSpinTask：D435 看到了新目标，先停住自转
//         // YawSpinTask 会进入 STABILIZING 状态，锁住位置和 yaw
//         // ========================================================
...
//         RCLCPP_WARN(
//           get_logger(),
//           "[VISION_FRONT] append D435 target: x=%.2f y=%.2f z=%.2f total=%zu -> pause yaw scan %.2fs",
//           x, y, z,
...
//     // 3. TSP：使用 D435 扫描得到的 detected_targets
//     //    到每个目标附近时再开启下视工业相机做视觉矫正
...
//     sched_.add(std::make_unique<ReturnHomeTask>(
//       get_logger(),
//       return_step_xy_,
//       return_xy_tol_,
//       return_vxy_tol_));
...
//     // ============================================================
//     // D435 发现目标后，YawSpinTask 会进入 STABILIZING
//     // 这里等稳定采样时间到了，再置 vision_done=true
//     // ============================================================
...
//     // 后期去重：只在控制端进入 detected_targets 前做一次 XY 去重
//     for (const auto& p : ctx_.detected_targets) {
//       const float dx = p.x - x;
//       const float dy = p.y - y;
//       const float d = std::sqrt(dx * dx + dy * dy);

//       if (d < static_cast<float>(front_duplicate_xy_m_)) {
//         duplicate = true;
//         break;
//       }
//     }
...
//     RCLCPP_WARN(
//       get_logger(),
//       "[VISION_FRONT] append D435 target: x=%.2f y=%.2f z=%.2f total=%zu",
//       x, y, z,
//       ctx_.detected_targets.size());
//   }

//   // 这一帧里只要加入了至少一个新目标，就通知 YawSpinTask 暂停一次
//   if (appended_count > 0) {
//     ctx_.vision_detected = true;
//     ctx_.vision_done = false;
...
//     RCLCPP_WARN(
//       get_logger(),
//       "[VISION_FRONT] frame accepted: new=%zu duplicate=%zu invalid=%zu visible=%zu total=%zu -> pause yaw scan %.2fs",
//       appended_count,
//       duplicate_count,
//       invalid_count,
//       msg->targets.size(),
...
//     *this->get_clock(),
//     1000,
//     "[VISION_FRONT] no new target: duplicate=%zu invalid=%zu visible=%zu total=%zu",
//     duplicate_count,
//     invalid_count,
//     msg->targets.size(),
//     ctx_.detected_targets.size());
...
//   const custom_vision_msgs::msg::PositionTargetArray::SharedPtr msg)
// {
//   // 只有 YawSpin2Task 打开 D435 采样窗口时，才接受深度相机目标点
//   if (!ctx_.vision_front_enable) {
//     return;
//   }

//   // 当前 45 度扇区已经进入稳定采样状态后，允许持续接收多帧；
//   // 但如果这一轮采样已经完成，则不再接收，避免重复加入。
//   if (ctx_.vision_done) {
//     return;
//   }
...
//     RCLCPP_WARN(
//       get_logger(),
//       "[VISION_FRONT][YawSpin2] append target: x=%.2f y=%.2f z=%.2f total=%zu",
//       x, y, z,
//       ctx_.detected_targets.size());
//   }

//   // YawSpin2 的逻辑不是“发现目标就让飞机停”，
//   // 因为飞机已经由 YawSpin2 停在当前 45 度扇区了。
//   // 这里的作用是：当前采样窗口内看到了图像，并开始计时稳定采样。
//   if (!front_stabilizing_) {
//     front_stabilizing_ = true;
//     front_stabilize_start_ = this->now();
...
//     RCLCPP_WARN(
//       get_logger(),
//       "[VISION_FRONT][YawSpin2] sampling window started: visible=%zu new=%zu duplicate=%zu invalid=%zu total=%zu, wait %.2fs",
//       msg->targets.size(),
//       appended_count,
//       duplicate_count,
//       invalid_count,
...
//     *this->get_clock(),
//     1000,
//     "[VISION_FRONT][YawSpin2] sampling: new=%zu duplicate=%zu invalid=%zu visible=%zu total=%zu",
//     appended_count,
//     duplicate_count,
//     invalid_count,
//     msg->targets.size(),
